...
gibbs_DL_confounder = function(y, X, Z ,nrun, burn, thin = 1, 
   ...
   for(i in 1:nrun){
      ...
      for (h in 1:n){                # Metropolis hastings step 
         ...
         logr = eta_star.T%*%(aMH - 2*Psi*(y[h] - mu_z[h])/sigmasq_y)%*%eta_star -
            2*eta_star.T%*%(Lambda.T%*%diag(ps)%*%X[h,] + phi*(y[h]- mu_z[h])/sigmasq_y) +
            2*eta_star.T%*%phi*(eta_star%*%Psi%*%eta_star)/sigmasq_y + 
            (1/sigmasq_y)*(eta_star%*%Psi%*%eta_star)^2 - 
            (eta.T%*%(aMH - 2*Psi*(y[h]- mu_z[h])/sigmasq_y)%*%eta[h,] -
                2*eta.T%*%(Lambda.T%*%diag(ps)%*%X[h,] + phi*(y[h]- mu_z[h])/sigmasq_y) +
