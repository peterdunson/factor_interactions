...
gibbs_DL_confounder_int = function(y, X, Z ,nrun, burn, thin = 1, 
   ...
   VZ = apply(Z, 2, var)
   dsVZ = diag(sqrt(VZ)); dsVZ_inv = diag(1/sqrt(VZ))
   ...
   for(i in 1:nrun){
      ...
      for (h in 1:n){                # Metropolis hastings step 
         ...
         phi_upd = as.vector(phi + Phi%*%Z[h,])
         aMH_upd = aMH + (Phi%*%Z[i,])%*%t(Phi%*%Z[i,])/sigmasq_y
         eta_star = bayesSurv::rMVNorm(1,eta[h,],diag(k)*delta_rw)
         eta_star.T = t(eta_star)     # avoid repeated transpose calls
         eta.T = t(eta[h,])
         
         logr = eta_star.T%*%(aMH_upd - 2*Psi*(y[h] - mu_z[h])/sigmasq_y)%*%eta_star -
            2*eta_star.T%*%(Lambda.T%*%diag(ps)%*%X[h,] + phi_upd*(y[h]- mu_z[h])/sigmasq_y) +
            2*eta_star.T%*%phi_upd*(eta_star%*%Psi%*%eta_star)/sigmasq_y + 
            (1/sigmasq_y)*(eta_star%*%Psi%*%eta_star)^2 - 
            (eta.T%*%(aMH_upd - 2*Psi*(y[h]- mu_z[h])/sigmasq_y)%*%eta[h,] -
                2*eta.T%*%(Lambda.T%*%diag(ps)%*%X[h,] + phi_upd*(y[h]- mu_z[h])/sigmasq_y) +
                2*eta.T%*%phi_upd*(eta[h,]%*%Psi%*%eta[h,])/sigmasq_y +
      ...
      # --- Update sigmasq_y --- #
      an = 0.5 + n/2
      bn = 0.5 + 0.5*t(y-eta%*%phi-diag(eta%*%Psi%*%eta.T)-diag(eta%*%Phi%*%Z.T)-mu_z)%*%
         (y-eta%*%phi-diag(eta%*%Psi%*%eta.T)-mu_z-diag(eta%*%Phi%*%Z.T))
      ...
      if ((i %% thin == 0) & (i > burn)){
         ...
         Omega_conf[count,,] = dsVX_inv%*%t(a_n)%*%Phi%*%dsVZ_inv
