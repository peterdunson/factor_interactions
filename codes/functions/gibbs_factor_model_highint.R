gibbs_factor_model_highint = function(y, X ,nrun = 2000, burn = 1000, thin = 1, 
   ...
   k_st = numeric(sp)
   beta_st2 = beta_bayes = beta_bayes3 = matrix(0,sp,p)
   beta_int = array(0,c(sp,p,p,p))
   ...
   Plam = psijh*matrix(rep(1,p),p,k,byrow=T)
   # output
   ...
   for(s in 1:nrun){
      ...
      #new RW MH
      MH = MH_RW_noZ3(phi,phi3,sigmasq_y,Lambda,ps,k,X,y,eta,acp,s,burn,Psi,delta_rw)
      eta = MH[[1]]
      acp = MH[[2]]
      ...
      Vcsi = solve(Lambda_n)
      Mcsi = Vcsi%*%t(X_reg)%*%(y-eta%*%phi-(eta^3)%*%phi3)/sigmasq_y
      ...
      Vcsi = solve(Lambda_n)
      Mcsi = Vcsi%*%t(eta)%*%(y-diag(eta%*%Psi%*%t(eta))-(eta^3)%*%phi3)/sigmasq_y
      phi = as.numeric(rmvnorm(1,Mcsi,Vcsi))
      
      # update phi3
      Lambda_n = t(eta^3)%*%(eta^3)/sigmasq_y + diag(rep(1,ncol(eta)))/100
      Vcsi = solve(Lambda_n)
      Mcsi = Vcsi%*%t(eta^3)%*%(y-diag(eta%*%Psi%*%t(eta))-(eta)%*%phi)/sigmasq_y
      phi3 = as.numeric(rmvnorm(1,Mcsi,Vcsi))
      
      #update sigmasq y when updating all together alpha phi and lambda
      an = 0.5 + n/2
      bn = 0.5 + 0.5*t(y-eta%*%phi-(eta^3)%*%phi3-diag(eta%*%Psi%*%t(eta)))%*%
         (y-eta%*%phi-(eta^3)%*%phi3-diag(eta%*%Psi%*%t(eta)))
      ...
      psijh = psijh*(1/(df/2+(Lambda^2)*matrix(rep(1,p),p,k,byrow = T)))
      ...
      if ((s%%thin==0)&(s>burn)){
         ...
         beta_bayes[s-burn,] = as.vector(t(phi)%*%a_n) + diag(V_n)%*%a_n
         beta_bayes3[s-burn,] = as.vector(t(phi3)%*%a_n^3)
         
         for(l in 1:p){
            for(h in (l+1):p){
               for(f in (h+1):p){
                  if(h < p+1 & f < p+1){
                     beta_int[s-burn,l,h,f] = sum(6*phi3*a_n[,l]*a_n[,h]*a_n[,f])  
                  }
               }
            }
         }
